...
| #15 | En tant que développeur, je souhaite pouvoir supprimer une **Task** d'un projet si elle a le status "TODO" afin de l'enlever du travail à faire. (Undo-able) | Moyenne-Haute | 3 |DOING|
| #29 | En tant que Scrum Master, je souhaite pouvoir publier une **release** (se compose de : dossier compressé contenant le code fonctionnel du projet, une description de ce qui a été fait, un numéro de version générer automatiquement grace à un rapide questionnaire (numero du sprint concerné (entier), checkbox pour le type de mofication: majeure/mineure)) afin de la rendre disponible pour les utilisateurs. | Basse | 3 |DOING|
